...
export default function ChangeLanguageButton({translations}: TranslationProps) {
    ...
    return (
        <button className="flex flex-col w-[62px] h-[50%] p-[10px] bg-gradient-to-b from-[#202D31] from-0% via-[#273439] via-59% to-[#1B292E] to-100%
            ...
            {showTranslations && translations?.map((version, index) => (
                <Link
                    key={version}
                    href={pathname.split('/').length > 2 ? `/${version}/${pathname?.split('/')[2]}` : `/${version}`}
                    locale={version}
